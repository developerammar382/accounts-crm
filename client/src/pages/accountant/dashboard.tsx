...
import type { Business, Document } from "@shared/schema";
...
export default function AccountantDashboard() {
  ...
  // Calculate metrics
  const totalClients = new Set(clientBusinesses.map((b: Business) => b.userId)).size;
  const totalBusinesses = clientBusinesses.length;
  const pendingTasks = documentsAwaitingReview.length;
  
  // Mock additional metrics
  const totalClientRevenue = clientBusinesses.reduce((sum: number, business: Business) => sum + 125000, 0);
  const vatReturnsDue = clientBusinesses.filter((b: Business) => b.vatScheme !== 'not_registered').length;

  // Recent client activity (mocked based on document uploads)
  const recentActivity = documentsAwaitingReview.slice(0, 3).map((doc: Document) => ({
    id: doc.id,
    type: 'document_upload',
    description: `${doc.fileName} uploaded`,
    businessName: 'Client Business', // Would be joined in real app
  ...
  const urgentTasks = [
    {
      ...
      action: 'Review',
      daysLeft: 5,
      onClick: () => setLocation('/accountant/vat-returns')
    },
    {
      id: 'document-review',
      ...
      onClick: () => setLocation('/accountant/document-review')
    },
    {
      id: 'client-approval',
      ...
      onClick: () => setLocation('/accountant/cis-returns')
    }
  ];

  // Client overview data (top clients)
  const topClients = clientBusinesses.slice(0, 4).map((business: Business) => ({
    id: business.id,
    name: business.name,
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold">{documentsAwaitingReview.length}</p>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Button 
                ...
                onClick={() => setLocation('/accountant/clients')}
              >
                View All Clients
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">
                                {client.type?.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                              </p>
                            </div>
                          </div>
                        </td>
